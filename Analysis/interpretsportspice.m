...
sportData = fullfile(boxpath,"LTspice","langmuir","SPORTmodelV5.txt");
data = loadspicefile(sportData);
save(fullfile(boxpath,"LTspice","langmuir","SPORTmodelV5.mat"),"data")
...
for i = 1:height(areaarray)
    d2 = data(data.steps == areaarray(i),:);
    [t,n] = gettempdensityest(d2.appliedpotential,-d2.("Ix(LP:Plasma)"),true,areaarray(i),density);
